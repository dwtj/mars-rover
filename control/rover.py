...
class Environment():
    ...
    def finalize_contours(self):
        ...
        self.contours.concatenate(self.volatile_contours)
        self.clear_volatile_contours()
        # No changes to the `Environment` plot should be necessary.




    def _clear_volatile_contours():
    ...
    def update_scan_field(self):
        ...
        w = self.scan_field
        if w != None:
            self.view.artists.remove(region)

        # Make a semi-circle with radius 100 at the current rover location:
        w = Wedge(self.loc, 100, self.direction - 90, self.direction + 90)
        w.set_facecolor('0.15')  # grey
